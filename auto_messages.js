...
class RocketChatAutomation {
  constructor(serverUrl, username, password, dannyUsername) {
    ...
    // Initialize/persist shuffle state
  ...
  // …then paste the rest (sendSafetyMessage, startAutomation, etc.) below…  
}
...
(async () => {
  ...
  async sendSafetyMessage() {
    ...
    const now = DateTime.now().setZone('America/Chicago');
    if (now.hour < 10 || (now.hour === 19 && now.minute > 30) || now.hour > 19) return;
    ...
    const roomId = resInfo.data.room._id;

    // Pick next message in the shuffled list
    const { order, index } = this.state;
    const msgIndex = order[index % order.length];
    const message  = this.safetyMessages[msgIndex];
  ...
  async sendHeatReminderMessage() {
    ...
    const message =
      ...
      `The Texas heat is no joke. Aim to finish half your route by 2 PM so you can cool off before it peaks.`;
    await this.sendMessage(roomId, message);
  }
  ...
  async sendClockInReminderMessage() {
    ...
    // Reminder always goes to today's room
    const roomName = this.getCurrentRoomName();
    const resInfo  = await axios.get(
      `${this.serverUrl}/api/v1/rooms.info?roomName=${encodeURIComponent(roomName)}`,
      { headers: { 'X-Auth-Token': this.authToken, 'X-User-Id': this.userId } }
  ...
  // ─── Startup Scheduler ────────────────────────────────────────────────────
  startAutomation() {
    // Safety: every 30 min from 10:00–19:30 CT
    ...
    // Heat reminder: 9:00 CT, May–Sep
    cron.schedule('0 9 * 5-9 *', () => this.sendHeatReminderMessage(), { timezone: 'America/Chicago' });

    // Clock‑in reminder: daily 9:25 CT
    cron.schedule('25 9 * * *', () => this.sendClockInReminderMessage(), { timezone: 'America/Chicago' });

    // Send a quick test DM to Danny
    (async () => {
      if (!this.authToken) await this.authenticate();
      const dmRoom = await this.getOrCreateDirectMessageRoom(this.dannyUsername);
      if (dmRoom) {
        await this.sendMessage(dmRoom, '✅ Safety Automation Deployed Successfully.');
      }
    })();
  }
} // ───────────────── end of RocketChatAutomation class ──────────────────────
