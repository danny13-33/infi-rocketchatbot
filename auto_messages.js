...
class RocketChatAutomation {
  constructor(serverUrl, username, password, dannyUsername) {
    ...
    // persisted state { date, order: [indices], index }
...
Let’s stay safe and smart out there!`
    ...
    // initialize or load existing shuffle state
  ...
  // fetch or create the daily room
  async getRoomId(roomName) {
    ...
    try {
      ...
      return create.data.channel._id;
    }
  }
  ...
  startAutomation() {
    // test DM to Danny
    (async () => {
      const dm = await this.getRoomId(this.dannyUsername);
    ...
    // Safety: every 30m from 10:00–19:30 CT
    cron.schedule(
      '0,30 10-19 * * *',
