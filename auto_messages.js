...
class RocketChatAutomation {
    constructor(serverUrl, username, password, dannyUsername) {
        ...
        // Persisted state will include { date, order, index, usedImages, sentMessages }
        this.state = {};
        this.dailyOrder = [];
        this.messageIndex = 0;
        this.sentMessages = {};
        ...
        this.safetyMessages = [
            ...
            `📌 Reminder: Try to avoid reversing whenever possible. If you must
        ];
    ...
    startAutomation() {
        ...
        // Saturday 5pm final timecard reminder
    ...
    stopAutomation() {
        if (this.scheduledSafetyTask) this.scheduledSafetyTask.stop();
        if (this.scheduledHydrationTask) this.scheduledHydrationTask.stop();
        if (this.scheduledHeatReminderTask) this.scheduledHeatReminderTask.stop();
        if (this.scheduledFridayTask) this.scheduledFridayTask.stop();
        if (this.scheduledSaturdayTask) this.scheduledSaturdayTask.stop();
        if (this.scheduledLunchReminderTask) this.scheduledLunchReminderTask.stop();
        if (this.scheduledDeliveryCountdown1130) this.scheduledDeliveryCountdown1130.stop();
        if (this.scheduledDeliveryCountdown1330) this.scheduledDeliveryCountdown1330.stop();
        if (this.scheduledDeliveryCountdown1530) this.scheduledDeliveryCountdown1530.stop();
        if (this.scheduledDeliveryCountdown1730) this.scheduledDeliveryCountdown1730.stop();
        if (this.scheduledImageUpload1) this.scheduledImageUpload1.stop();
        if (this.scheduledImageUpload2) this.scheduledImageUpload2.stop();
        if (this.scheduledImageUpload3) this.scheduledImageUpload3.stop();
    }
}
