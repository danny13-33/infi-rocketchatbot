...
Let’s stay safe and smart out there!`
  ...
  loadOrInitState() {
    ...
    if (
      ...
      this.state = { date: today, order: arr, index: 0 };
  ...
  async sendImmediateMessageToDanny() {
    const { dannyUsername } = this;
    const res = await axios.post(
      `${this.serverUrl}/api/v1/im.create`,
      { username: dannyUsername },
      { headers: { 'X-Auth-Token': this.authToken, 'X-User-Id': this.userId } }
    );
    const dmId = res.data.room._id;
    await this.postMessage(dmId, `✅ Safety Automation Deployed Successfully.\nThis is your immediate test message, Danny.`);
  }

  async sendSafetyMessage() {
    console.log('🔒 Safety message');
    const room = this.getCurrentRoomName();
    const id = await this.checkRoomExists(room);
    if (!id) return;
    await this.postMessage(id, this.getNextSafetyMessage());
  }
  ...
  async sendHydrationMessage() {
    ...
    const id = await this.checkRoomExists(room);
    if (!id) return;
    await this.postMessage(id, `🌊 HYDRATE 🌊\nIf you are reading this, drink water now!`);
  }

  async sendHeatReminderMessage() {
    console.log('🔥 Heat reminder');
    const m = DateTime.now().setZone('America/Chicago').month;
    if (m < 5 || m > 9) return;
    const room = this.getCurrentRoomName();
    const id = await this.checkRoomExists(room);
    if (!id) return;
    await this.postMessage(id, `@all ⚠️ Texas heat reminder ⚠️\nKnock out >50% of your route by 2 PM so you can cool off safely.`);
  }

  async sendClockInReminderMessage() {
    console.log('⏲️ Clock-in reminder');
    const room = this.getCurrentRoomName();
    const id = await this.checkRoomExists(room);
    if (!id) return;
    await this.postMessage(id, `@all *Clock-In Reminder*\nPlease clock in now or email time@infi-dau7.com if you’re blocked.`);
  }

  async sendFridayTimecardReminder() {
    console.log('📆 Friday timecard');
    const info = await axios.get(`${this.serverUrl}/api/v1/rooms.info?roomName=general`, {
      headers: { 'X-Auth-Token': this.authToken, 'X-User-Id': this.userId }
    });
    const id = info.data.room._id;
    await this.postMessage(id, `@all *Friday Timecard Check*\nVerify your time punches. If corrections needed, email time@infi-dau7.com.`);
  }

  async sendSaturdayTimecardReminder() {
    console.log('📆 Saturday timecard');
    const info = await axios.get(`${this.serverUrl}/api/v1/rooms.info?roomName=general`, {
      headers: { 'X-Auth-Token': this.authToken, 'X-User-Id': this.userId }
    });
    const id = info.data.room._id;
    await this.postMessage(id, `@all *Final Timecard Reminder*\nSubmit corrections by midnight tonight if needed.`);
  }

  async sendDeliveryCountdownReminder(hoursLeft, label) {
    console.log(`⏰ ${label} countdown (${hoursLeft}h left)`);
    const room = this.getCurrentRoomName();
    const id = await this.checkRoomExists(room);
    if (!id) return;
  ...
  async sendLunchReminderMessage() {
    console.log('🍽️ Lunch reminder');
    const room = this.getCurrentRoomName();
    const id = await this.checkRoomExists(room);
    if (!id) return;
    await this.postMessage(id, `@all 🍽️ Lunch Time!\nMandatory 30 min—hit the Break button in Flex, enjoy & recharge!`);
  }
  ...
  startAutomation() {
    ...
    cron.schedule('30 11 * * *', () => {
      this.sendDeliveryCountdownReminder(7, '11:30').catch(console.error);
    }, { timezone: 'America/Chicago' });

    cron.schedule('30 13 * * *', () => {
      this.sendDeliveryCountdownReminder(5, '13:30').catch(console.error);
    }, { timezone: 'America/Chicago' });

    cron.schedule('30 15 * * *', () => {
      this.sendDeliveryCountdownReminder(3, '15:30').catch(console.error);
    }, { timezone: 'America/Chicago' });

    cron.schedule('30 17 * * *', () => {
      this.sendDeliveryCountdownReminder(1, '17:30').catch(console.error);
    }, { timezone: 'America/Chicago' });
