...
class RocketChatAutomation {
  ...
  saveOrInitState() {} // <–– not used

  saveState() {
    fs.writeFileSync(STATE_PATH, JSON.stringify(this.state, null, 2), 'utf8');
  }

  // ─── Full list of safety messages (no omissions!) ───────────────────────────
  get safetyMessages() {
    return [
...
Let’s stay safe and smart out there!`
  ...
  async sendHeatReminderMessage() {
    ...
    const msg =
      ...
      'Texas heat is no joke. Knock out more than half your route by 2 PM to beat the worst temperatures. Keep moving, stay organized, and manage your time wisely. You’ve got this Titans! 💪🔥';
    await this.sendMessage(roomId, msg);
  }
  ...
  async sendFridayTimecardReminder() {
    ...
    const msg =
      ...
      "Here's your reminder to check and ensure your timecard is accurate. If you missed a punch, email time@infi-dau7.com with:\n\n" +
      'Date:\nClock in:\nLunch out:\nLunch in:\nClock out:\n\n' +
  ...
  async sendSaturdayTimecardReminder() {
    ...
    const msg =
      '@all *Final Reminder*\n' +
      "Did you remember to check your timecard? Corrections due by midnight. If needed, email time@infi-dau7.com with:\n\n" +
      'Date:\nClock in:\nLunch out:\nLunch in:\nClock out:\n\n' +
      '*DO NOT USE ADP TO CORRECT YOUR TIMECARD*';
    await this.sendMessage(roomId, msg);
  }
  ...
  async sendLunchReminderMessage() {
    ...
    const msg =
      ...
      'Lunches are mandatory and must be exactly 30 minutes. ⏳\n➔ No more, no less.\n❌ Cannot combine with breaks.\n🚗 Travel time counts as part of your 30-minute lunch.\n\n' +
      'Hit the Break button in the Flex app before you dig in! ✅\nEnjoy and recharge! 💪🥗🍔';
    await this.sendMessage(roomId, msg);
  }
