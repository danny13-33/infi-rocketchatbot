...
Let’s stay safe and smart out there!`
  ...
  loadOrInitState() {
    ...
    if (
      ...
      typeof saved.usedImages === 'object'
    ) {
      this.state = saved;
    } else {
      ...
      this.state = { date: today, order, index: 0, usedImages: {} };
      this.saveState();
    }
  }
  ...
  getCurrentRoomName() {
    return `daily-${DateTime.now().setZone('America/Chicago').toISODate()}`;
  }
  ...
  async sendSafetyMessage() {
    ...
    let name = this.getCurrentRoomName();
    let id = await this.checkRoomExists(name) || await this.createRoom(name);
    await this.sendMessage(id, this.getNextSafetyMessage());
  }
  ...
  async sendHydrationMessage() {
    ...
    if (m < 5 || m > 9) return;
    let name = this.getCurrentRoomName();
    let id = await this.checkRoomExists(name);
    if (id) {
      await this.sendMessage(id, '🌊 HYDRATE NOW! Stay hydrated to beat the heat.');
    }
  }
  ...
  async sendHeatReminderMessage() {
    ...
    let id = await this.checkRoomExists(name);
    if (id) {
      await this.sendMessage(id, '@all ⚠️ Titans! Finish half your route by 2 PM to beat the heat.');
    }
  }
  ...
  async sendClockInReminderMessage() {
    ...
    let id = await this.checkRoomExists(name);
    if (id) {
      await this.sendMessage(id, '@all *Reminder:* Clock-in now or email time@infi-dau7.com.');
    }
  }

  async sendFridayTimecardReminder() {
    if (!(await this.ensureAuth())) return;
    const r = await axios.get(`${this.serverUrl}/api/v1/rooms.info?roomName=general`, {
      headers: { 'X-Auth-Token': this.authToken, 'X-User-Id': this.userId }
    });
    await this.sendMessage(r.data.room._id, '@all *Friday:* Check your timecard; email corrections to time@infi-dau7.com.');
  }
  ...
  async sendSaturdayTimecardReminder() {
    ...
    await this.sendMessage(r.data.room._id, '@all *Final:* Last chance to correct timecard—due by midnight.');
  }
  ...
  async sendLunchReminderMessage() {
    ...
    let id = await this.checkRoomExists(name);
    if (id) {
      await this.sendMessage(id, '@all 🍽️ Lunch time! Must be exactly 30 min—hit Break in the Flex app.');
    }
  }
  ...
  async sendDeliveryCountdownReminder1130() {
    ...
    let id = await this.checkRoomExists(name);
    if (id) {
      await this.sendMessage(id, '@all *7h left!* Keep pace to finish before 6:30 PM.');
    }
  }
  ...
  async sendDeliveryCountdownReminder1330() {
    ...
    let id = await this.checkRoomExists(name);
    if (id) {
      await this.sendMessage(id, '@all *5h left!* Push to finish on time.');
    }
  }
  ...
  async sendDeliveryCountdownReminder1530() {
    ...
    let id = await this.checkRoomExists(name);
    if (id) {
      await this.sendMessage(id, '@all *3h left!* Almost there—keep moving.');
    }
  }
  ...
  async sendDeliveryCountdownReminder1730() {
    ...
    let id = await this.checkRoomExists(name);
    if (id) {
      await this.sendMessage(id, '@all *1h left!* Final push—complete safely.');
    }
  }

  async sendRandomImageReminder() {
    if (!(await this.ensureAuth())) return;
    const imgs = ['dogs.jpg','leadwithsafety.jpg','stopsigns.jpg'];
    const today = DateTime.now().setZone('America/Chicago').toISODate();
    const used = this.state.usedImages[today] || [];
    const avail = imgs.filter(i => !used.includes(i));
    if (!avail.length) return;
    const pick = avail[Math.floor(Math.random()*avail.length)];
    await this.uploadImageToDailyRoom(pick);
    this.state.usedImages[today] = [...used, pick];
    this.saveState();
  }

  async uploadImageToDailyRoom(img) {
    let name = this.getCurrentRoomName();
    let id = await this.checkRoomExists(name);
    if (!id) return;
    const form = new FormData();
    form.append('file', fs.createReadStream(path.join(__dirname,'images',img)));
  ...
  startAutomation() {
    // Send immediate test DM
    this.authenticate().then(ok => {
      if (!ok) return;
      this.getOrCreateDirectMessageRoom(this.dannyUsername)
        .then(dm => { if (dm) this.sendMessage(dm, '✅ Safety Automation Deployed. Test message.'); })
        .catch(console.error);
    });
