...
class RocketChatAutomation {
  ...
  // Load or initialize today's shuffle
  loadOrInitState() {
  ...
  listSafetyMessages() {
    ...
    this.safetyMessages.forEach((msg, idx) => {
  ...
  // Generate today's room name, e.g. "July-30th-2025"
  getCurrentRoomName() {
  ...
  // …and here we'll pause. Send “continue” for the rest (sending methods, cron setup, startup, etc.).
::contentReference[oaicite:0]{index=0}
  // Generic method to post a message to a room
  async sendMessage(roomId, text) {
    try {
      ...
      console.log(`📤 Sent to ${roomId}: ${text.substring(0, 30)}…`);
    } catch (err) {
      console.error('❌ sendMessage error:', err.response?.data || err.message);
    }
  }

  // Send the next safety tip if within business hours
  ...
  // Hourly hydration in summer months
  async sendHydrationMessage() {
    ...
    await this.sendMessage(
      roomId,
      `🌊 HYDRATE 🌊  Drink water now! Stay safe in the heat.`
    );
  }

  // Daily heat reminder at 9 AM in summer
  async sendHeatReminderMessage() {
    ...
    await this.sendMessage(
      roomId,
      `@all ⚠️ Titans—beat the Texas heat! Aim to finish half your route by 2 PM. 💪`
    );
  }

  // Clock‑in prompt at 9:25 AM
  async sendClockInReminderMessage() {
    ...
    await this.sendMessage(
      roomId,
      `@all ⏰ Please clock in now. If you have issues, email time@infi-dau7.com.`
    );
  }

  // Get or open a direct‑message room with a user
  async getOrCreateDirectMessageRoom(username) {
    try {
      ...
      console.error('❌ DM room error:', err.response?.data || err.message);
      return null;
    }
  }

  // Send Danny an immediate confirmation DM
  async sendImmediateMessageToDanny() {
    ...
    await this.sendMessage(
      dmId,
      `✅ Safety Automation Deployed Successfully. This is your test message, Danny.`
    );
  }

  // Schedule all tasks
  startAutomation() {
    // fire-and-forget startup DM
    this.sendImmediateMessageToDanny().catch(console.error);

    // Safety every 30m from 10:00 to 19:30 CT
...
// ─── Bootstrap ────────────────────────────────────────────────────────────────
console.log('🔧 Starting RocketChatAutomation…');
console.log({
