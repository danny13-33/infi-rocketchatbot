...
Let’s stay safe and smart out there!`
  ...
  // ... loadOrInitState(), saveState(), listSafetyMessages()
  ...
  // ... remaining message methods and startAutomation()
}

// Export or instantiate below as needed
  // Load or initialize the shuffled order for today's safety messages
  loadOrInitState() {
    const today = DateTime.now().setZone('America/Chicago').toISODate();
    let persisted = null;
  ...
  // Helpers to get today's room name (e.g. "July-30th-2025")
  ...
  getCurrentRoomName() {
    ...
    return `${now.monthLong}-${now.day}${this.getOrdinalSuffix(now.day)}-${now.year}`;
  }

  // Ensure room exists, else skip
  ...
  // Send the next safety reminder (every 30m during business hours)
  async sendSafetyMessage() {
    const now = DateTime.now().setZone('America/Chicago');
    const minutes = now.hour * 60 + now.minute;
    if (minutes < 10*60 || minutes > 19*60 + 30) return;
  ...
  // Hydration reminders (hourly from 10–18, May–Sept)
  ...
  // Heat-of-summer morning reminder (9:00am May–Sept)
  async sendHeatReminderMessage() {
    ...
    await this.sendMessage(roomId,
      ...
      `By hustling early, you’ll give yourself the chance to slow down and cool off when the heat is at its worst. Keep moving, stay organized, and manage your time wisely.\n\n` +
      `You’ve got this Titans! 💪🔥`
    );
  }

  // Clock-in reminder (9:25am daily)
  async sendClockInReminderMessage() {
    if (!this.authToken) await this.authenticate();
    const roomId = await this.checkRoomExists(this.getCurrentRoomName());
    if (!roomId) return;
    await this.sendMessage(roomId,
      `*Attention Titans*\n@all This is your daily reminder to clock in. If you are unable, email time@infi-dau7.com immediately. Thank you!`
    );
  }

  // Friday timecard check (8:00am Fridays in #general)
  async sendFridayTimecardReminder() {
    ...
    await this.sendMessage(roomId,
      `@all *Attention Titans*\nHere's your reminder to check and ensure your timecard is accurate. If not, email time@infi-dau7.com with:\n\nDate:\nClock in:\nLunch out:\nLunch in:\nClock out:\n\n*DO NOT USE ADP TO CORRECT YOUR TIMECARD*`
    );
  }

  // Saturday final timecard push (5:00pm Saturdays)
  async sendSaturdayTimecardReminder() {
    ...
    const roomId = res.data.room._id;
    await this.sendMessage(roomId,
      `@all *Final Reminder*\nDid you remember to check your timecard? Corrections due by midnight tonight. Email time@infi-dau7.com in the specified format.`
    );
  }

  // Lunch-time push (2:00pm daily)
  async sendLunchReminderMessage() {
    ...
    await this.sendMessage(roomId,
      ...
      `Lunches are mandatory and exactly 30 minutes — no more, no less. 🚗 Travel counts in that 30. Hit Break in the Flex app before you dig in! ✅`
    );
  }

  // Delivery countdown reminders
  async sendDeliveryCountdownReminder1130() { await this._countdown(7, '30 11 * * *'); }
  async sendDeliveryCountdownReminder1330() { await this._countdown(5, '30 13 * * *'); }
  async sendDeliveryCountdownReminder1530() { await this._countdown(3, '30 15 * * *'); }
  async sendDeliveryCountdownReminder1730() { await this._countdown(1, '30 17 * * *', true); }

  async _countdown(hoursLeft, cronSpec, lastHour = false) {
    if (!this.authToken) await this.authenticate();
    const roomId = await this.checkRoomExists(this.getCurrentRoomName());
    if (!roomId) return;
    const msg = lastHour
      ? `@all *Attention Titans!* Last hour remaining! 💥 Let's push through and complete the delivery day safely!`
      : `@all *Attention Titans!*\n\nYou have ${hoursLeft} hours left in your delivery day. Keep the pace to finish before 6:30pm. You got this! 💪`;
    await this.sendMessage(roomId, msg);
  }

  // Random image uploads (10:15, 12:15, 3:15)
  async sendRandomImageReminder() {
    ...
    const used = this.state.usedImages[todayKey] || [];
    const remain = images.filter(i => !used.includes(i));
    if (remain.length === 0) return;

    const choice = remain[Math.floor(Math.random()*remain.length)];
    const form = new FormData();
    form.append('file', fs.createReadStream(path.join(__dirname,'images',choice)));
    ...
    this.state.usedImages[todayKey] = [...used, choice];
    this.saveState();
  }

  // Kick off all cron jobs
  startAutomation() {
  ...
  stopAutomation() {
    [
      ...
      this.scheduledLunchReminderTask,
